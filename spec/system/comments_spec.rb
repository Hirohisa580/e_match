...
RSpec.describe "Boards", type: :system do
  ...
  context 'コメントができるとき' do
    it 'コメントを入力すれば作成できる' do
      ...
      @board.save
      visit "/boards/#{@board.id}/comments"
      fill_in "", with: @comment.comment
      expect{
        find('input[name="commit"]').click
      }.to change { Comment.count }.by(1)
      expect(current_path).to eq "/boards/#{@board.id}/comments"
    end
  end

  context 'コメントができないとき' do
    it 'コメントが空だとコメントできない' do
      visit root_path
      @user.save
      expect(page).to have_content("ログイン")
      visit new_user_session_path
